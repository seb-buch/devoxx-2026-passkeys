...
<Slide>
  ...
  <SpeakerNotes>
    <p>
      ...
      Ça commence pareil : le client demande des options au serveur, et le serveur
      renvoie un challenge. Par contre, cette fois-ci, pas besoin d'envoyer les infos
      utilisateur — on ne crée pas de credential, on en utilise un existant.<br />
      Le navigateur appelle navigator.credentials.get() — c'est le pendant de
      credentials.create() pour l'enregistrement.<br />
      L'authenticator reçoit la demande. Il vérifie d'abord l'identité de l'utilisateur
      — empreinte, Face ID, PIN — puis il crée une assertion. L'assertion, c'est la
      preuve cryptographique : l'authenticator signe le challenge avec la clé privée
      qu'il a stockée lors de l'enregistrement.<br />
      Et c'est là qu'est la grande différence avec l'enregistrement : on ne génère pas
      de nouvelle paire de clés, on réutilise celle qui existe déjà. La clé privée ne
      quitte toujours pas l'authenticator — il renvoie juste la signature.<br />
      Le serveur reçoit cette signature et la vérifie avec la clé publique qu'il avait
      stockée à l'enregistrement. Il vérifie aussi le challenge, l'origin et le rp_id,
      comme pour l'enregistrement. Si tout est bon, l'utilisateur est authentifié.
    </p>
  </SpeakerNotes>
</Slide>
