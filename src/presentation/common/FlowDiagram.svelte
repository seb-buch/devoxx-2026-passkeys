<script lang="ts">
	...
	function computeBoundingBox(index: number, step: ComputeStep): {
		startX: number,
		startY: number,
		width: number,
		height: number
...
<svg
	viewBox={computeViewBox(flowDiagram)}
	class="diagram"
	aria-label={flowDiagram.title}
>
	<defs>
	...
	{#each flowDiagram.streamLines as streamLine, index (index)}
		...
		<image href={streamLine.iconPath} x={x - 0.5* style.streamHeaderIconSize} y="0"
		       width={style.streamHeaderIconSize} height={style.streamHeaderIconSize} />
		<text x={x} y={style.streamHeaderIconSize + 0.5 * style.streamHeaderLineHeight}
		      class="actor-name">{streamLine.name}</text>
	...
	{#each flowDiagram.steps as step, index (index)}
		<g class={useFragment ? "fragment" : null } data-fragment-index={ index }>
			{#if 'from' in step}
				...
				<line x1={startX} y1={y} x2={endX} y2={y} class="arrow" stroke={fill}
				      marker-end="url(#arrow-head)" />
				<text x={middleX} y={y - 7} class="arrow-label" fill={fill}>{step.label}</text>
				<circle cx={startX} cy={y} r="12" fill={fill} />
				<text x={startX} y={y} class="step num">{index + 1}</text>
			{:else }
				{@const { startX, startY, width, height } = computeBoundingBox(index, step)}
				{@const midX = startX + width / 2 }
				{@const midY = startY + height / 2 }
				{@const fill = '#4a90d9'}
				<rect x={startX} y={startY} width={width} height={height} rx="5" class="process-box" />
				<text x={midX} y={midY} class="process-label">{step.label}</text>
				<circle cx={startX} cy={startY} r="12" fill={fill} />
